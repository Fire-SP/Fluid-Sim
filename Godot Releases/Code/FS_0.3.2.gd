	...
	for i in finallistx:
		for j in finallisty:
			...
			if enable_erosion == true:
				...
				if air[i][j] <= 0.3 or air[i][j] >= 1.2:
					heightmap[i][j] += sediment[i][j]*1.1
...
func generate_map():
	for i in range(map_size+1):
		...
		for j in range(map_size+1):
			...
			height = abs(height)
			var rock_H = (rock.get_noise_2d(i,j))
			rock_H = abs(rock_H)

			heightmap[i].append(abs(height))
			rock_hardness[i].append(abs(rock_H)/10)
			air[i].append(0)
			sediment[i].append(0)
...
func _draw():
	for i in range(map_size):
		for j in range(map_size):
			...
			var heightcolor = ((heightmap[j][i])-45)/35
...
#				draw_rect(Rect2((j*size)+100,(i*size),x,y),Color(((heightcolor/2)+dryness/3)+0.05,heightcolor/(dryness)+0.001,aircolor))
				...
				if heightmap[j][i] > 45 and heightmap[j][i] < 50:
					draw_rect(Rect2((j*size)+100,(i*size),x,y),Color(dryness/3,heightcolor/(dryness)+0.001,aircolor))
				if heightmap[j][i] >= 50:
					draw_rect(Rect2((j*size)+100,(i*size),x,y),Color((heightcolor/3.5)+dryness/3,heightcolor,aircolor))
