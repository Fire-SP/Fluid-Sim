		...
		for j in finallisty:
			...
			if enable_erosion == true:
				if air[i][j] > 0.01 and sediment[i][j] < 0.05:
					...
					if sediment[i][j] > 0.05:
						sediment[i][j] = 0.05

				if air[i][j] <= 0.2 and sediment[i][j] < 0.05:
...
func _draw():
	for i in range(map_size):
		for j in range(map_size):
			...
			if aircolor > 0.7 and state[j][i] == 1:
...
#				draw_rect(Rect2((j*size)+100,(i*size),x,y),Color(((heightcolor/2)+dryness/3)+0.05,heightcolor/(dryness)+0.001,aircolor))
				...
				if heightmap[j][i] <= 45 and state[j][i] == 0:
					draw_rect(Rect2((j*size)+100,(i*size),x,y),Color((dryness/3)+aircolor*0.3,((heightcolor))+aircolor*0.3,aircolor*0.3))
				if heightmap[j][i] > 45 and heightmap[j][i] < 55 and state[j][i] == 0:
					draw_rect(Rect2((j*size)+100,(i*size),x,y),Color((dryness/3)+aircolor*0.3,((heightcolor)/(dryness)+0.001)+aircolor*0.3,aircolor*0.3))
				if heightmap[j][i] >= 55 and state[j][i] == 0:
					draw_rect(Rect2((j*size)+100,(i*size),x,y),Color(((heightcolor/3.5)+dryness/3)+aircolor*0.3,((heightcolor)/(dryness)+0.001)+aircolor*0.3,aircolor*0.3))
					
				if heightmap[j][i] <= 45 and state[j][i] == 1:
					draw_rect(Rect2((j*size)+100,(i*size),x,y),Color((dryness/3),((heightcolor)),aircolor))
				if heightmap[j][i] > 45 and heightmap[j][i] < 55 and state[j][i] == 1:
					draw_rect(Rect2((j*size)+100,(i*size),x,y),Color((dryness/3),(heightcolor)/(dryness)+0.001,aircolor))
				if heightmap[j][i] >= 55 and state[j][i] == 1:
					draw_rect(Rect2((j*size)+100,(i*size),x,y),Color(((heightcolor/3.5)+dryness/3),(heightcolor)/(dryness)+0.001,aircolor))
...
func pen():
	...
	mouse = (mouse/15)
	if mouse[0] <= 7 or mouse[0] >= 81 or mouse[1] >= 75:
		stop_click = true
	else:
